...
const FindVisaWidget = ({ onPreFlowNavigation }: any) => (
  <div className="p-4">Find Visa Widget</div>
);
...
const TopBar = ({ variant, flagIcon, isLoggedIn, onFlagClick, onLogoClick, onSearchClick, onMenuClick, isFixed, showSearchIcon }: any) => (
  <div className={`bg-white border-b p-4 flex items-center justify-between ${isFixed ? 'fixed top-0 left-0 right-0 z-50' : ''}`}>
    <div>Logo</div>
    {showSearchIcon && <button onClick={onSearchClick}>🔍</button>}
  </div>
);
...
const HomeScreen = () => {
  ...
  // Mock hooks - replace with actual hooks when available
  // const { countryListData, isSuccess: isCountryListSuccess } = useCountry();
  const countryListData: { response: Country[] } = { response: [] };
  const isCountryListSuccess = true;
  ...
  // } = useLocation();
  const nationality: Country | null = null;
  const residency: Country | null = null;
  const handleUpdateNationality = (country: Country) => {};
  const handleUpdateResidency = (country: Country) => {};
  const locationResponse: any = null;
  const countryIsoCode: string | null = null;
  const handleUpdateCountryIsoCode = (code: string) => {};
          ...
          >
            ...
            </div>
            {
              countryListData?.response && !isMobile && (
                <div className="relative md:block sm:hidden" style={{ zIndex: 2, width: isTablet ? "80%" : "60%" }}>
            ...
            {modalType === "searchDestination" && (
              <div className="w-full flex justify-start relative md:justify-start sm:justify-center" style={{ zIndex: 2, width: isMobile ? "100%" : isTablet ? "80%" : "60%" }}>
                {isMobile ? (
                  <SearchField
                    isMobile={isMobile}
